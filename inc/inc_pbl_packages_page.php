<?php
function pbl_packages_page() {
global $wpdb;

	if($_POST['action']=="add_package"){
		$name=mysql_real_escape_string(stripslashes($_POST['pbl_package_name']));
		$description=mysql_real_escape_string(stripslashes($_POST['pbl_package_description']));
		if($_POST['pbl_package_cost']==""){$cost=0;}else{$cost=mysql_real_escape_string(stripslashes($_POST['pbl_package_cost']));}
		$duration=mysql_real_escape_string(stripslashes($_POST['pbl_package_duration']));
		$wpdb->insert($wpdb->prefix.'pbl_packages',array('name'=>$name,'description'=>$description,'cost'=>$cost,'duration'=>$duration));
	}

	if($_POST['action']=="delete_package"){
		$id=$_POST['pbl_package_id'];
		$wpdb->query("DELETE FROM ".$wpdb->prefix."pbl_packages WHERE id=$id");
	}
?>
...
<?php
global $wpdb;
$packages=$wpdb->get_results("SELECT id,name,description,cost,duration FROM ".$wpdb->prefix."pbl_packages");
...
<form method="post"><input type="hidden" name="action" value="add_package" />
    <table class="form-table">
        ...
        <td><input type="text" name="pbl_package_name" value="" size="56" /></td>
        </tr>
         
        <tr valign="top">
        <th scope="row"><strong>Package Description</strong></th>
        <td><textarea name="pbl_package_description" cols="50" rows="2"></textarea></td>
        </tr>

        <tr valign="top">
        <th scope="row"><strong>Package Cost</strong></th>
        <td><input type="text" name="pbl_package_cost" value="" size="4" /> <strong><?php echo get_option('pbl_ppcurrency'); ?></strong> <em>(to change currencies, go to the <a href="?page=pbl_settings">General Settings</a> page)</em></td></tr>
        ...
        <td><input type="text" name="pbl_package_duration" value="" size="2" /> <strong>Months</strong></td>
        </tr>
